...
def get_recommendations(title):
    dataframe_places['Review Body'] = dataframe_places['Review Body'].apply(
        filter_keywords)
    dataframe_places["Review Body"] = dataframe_places["Review Body"].apply(
        lambda x: ' '.join(x))
    # a = a.reset_index()
    titles = dataframe_places['Place']
    indices = pd.Series(dataframe_places.index,
                        index=dataframe_places['Place'])
    idx = indices[title]
...
async def get_userid(picked_userid):
    ...
    recommendations = ranked_item_score['Place'].to_list()
